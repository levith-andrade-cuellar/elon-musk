...
# Replies -> Vader Sentiment Analysis

sentimentdf["replies_vader_compound"] = [] # Try the list method

replies_vader_compound = []

for replies_list in sentimentdf_withreplies['text_replies']:
    if replies_list != 0: 

        replies_vader_compound_list = []

        for reply in replies_list:
            compound_score = analyzer.polarity_scores(reply)['compound']
            replies_vader_compound_list.append(compound_score)
        
        replies_vader_compound.append(replies_vader_compound_list)

print(replies_vader_compound[i])

#for i in range(1):
    #sentimentdf["replies_vader_compound"][i] = replies_vader_compound[i]

sentimentdf_withreplies.to_csv("data.csv")              # Export the data as a CSV file. 


#print(sentimentdf_withreplies['text_replies'][0][0])